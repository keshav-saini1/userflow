...
import { FilterStep1, FilterStep2, FilterStep3, FilterStep4 } from '@/features/property-filter';
import type { FilterState } from '@/features/property-filter';
import { useFilterStore } from '@/features/property-filter';
...
export const SetupCard: React.FC<SetupCardProps> = ({ setShowBottomSheet }) => {
  const [currentFilterStep, setCurrentFilterStep] = useState<number | null>(null);
  const { filterState, updateFilter } = useFilterStore();
  ...
  const handleFilterNext = (data: Partial<FilterState>) => {
    updateFilter(data);
    ...
    <>
      <div className="bg-white rounded-[21px] border border-gray-200 shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1)] p-px overflow-hidden">
        <div className="p-[21px] flex flex-col gap-7">
          ...
          {/* Button Section */}
          <div className="flex gap-[10.5px] h-12 w-[340px]" onClick={() => handleFilterStep(1)}>
            {/* Search Button */}
              <button 
