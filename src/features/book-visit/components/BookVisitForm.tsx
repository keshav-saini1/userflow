...
const BookVisitForm: React.FC<BookVisitFormProps> = ({ onSuccess }) => {
  ...
  const isFormComplete = formData.visitType && formData.selectedDate && formData.selectedTime;
  ...
  const handleSubmit = () => {
    if (isFormComplete) {
      ...
      const bookingDetails = {
        ...
        visitType: formData.visitType === 'live-video-tour' ? 'Live Video Tour' : 
                   formData.visitType === 'visit-property' ? 'Visit Property' : 'Phone Call',
        date: formData.selectedDate ? formData.selectedDate.toLocaleDateString('en-US', {
          weekday: 'long',
          year: 'numeric',
          month: 'long',
          day: 'numeric'
